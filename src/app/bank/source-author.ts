...
export function sourceChapter(topicId:string,book:BookReference['book'],edition:string,chapter:number,pdfOffset:number|((printedPage:number)=>number),rows:SourceRow[],relatedPages:Record<number,number[]>={}):SourcedQuestion[]{
 return rows.map(([stem,options,key,page,section,learningObjective,optionRationales],i)=>{
  ...
  const reference={book,edition,chapter,printedPages,pdfPages:printedPages.map(p=>typeof pdfOffset==='number'?p+pdfOffset:pdfOffset(p)),section};
